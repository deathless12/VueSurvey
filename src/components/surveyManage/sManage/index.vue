...
export default {
  ...
  computed:{
  	clazzList(){
  		let vm = this;
  		return this.clazzVM.filter((item,index)=>{
  			if(item.grade){
  	...
  	...mapGetters(['surveies','naires','grades','teachers','courses','clazzVM']),
  },
  created(){
  ...
  methods:{
    ...
    getCurrentDate(){
  	...
  	...mapActions(['findAllSurvey','saveOneSurvey','deleteOneSurvey','batchSurvey','findAllClazzVM','findAllCourse','findAllTeacher','findAllGrade','findAllNaire']),
  }
}
</script>
<style>
